...
| `swapdex quota [--json]` | Read Claude and Codex account usage windows from their provider endpoints using the account’s own credential; no model request is submitted. Missing, rejected, throttled or unavailable readings remain explicit instead of becoming a guessed percentage. The dashboard also reads these endpoints. `--json` includes per-tool results and unexpected response shapes. A provider read failure is reported in the result. |
...
| `swapdex threshold <value\|off>` | Configure a usage threshold (`0.9` or `90%`) for leaving an account. Enabled thresholds add provider usage reads. Proxy model forwarding and eligible OAuth renewal are separate network activity. |
...
| `4` | Another Swapdex operation holds the required lock, or a requested refresh failed/was deferred (see `refresh`). |
