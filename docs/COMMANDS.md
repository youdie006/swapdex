...
| `swapdex ui` | Full-screen picker (real terminal): arrow keys, Enter to switch, `a` add a new account (the sign-in flow), `r` restore, `d` delete, `q` quit. After a switch: pick a recent session by number to resume it (in its own folder, via sessionwiki), or `c`/`x`/`g`/`a` to open a NEW claude/codex/gemini/agy conversation - it asks which folder. Switch, land in a conversation. Pipes fall back to a plain numbered prompt. |
