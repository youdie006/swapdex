...
| `swapdex ui` | Interactive picker: every profile with its account and active marker, plus the session summary; type a number to switch (the same safe `use` path), Enter or `q` cancels. Needs a terminal. |
