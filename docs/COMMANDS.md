...
| `swapdex quota [--json]` | Read Claude and Codex account usage windows from their provider endpoints using the account’s own credential; no model request is submitted. Eligible expired Claude slots can be renewed before the read, including slots with a same-name saved profile. Native sessions retain renewal ownership, and failed or deferred renewal is reported without sending the old token. Missing, rejected, throttled or unavailable readings remain explicit instead of becoming a guessed percentage. The dashboard also reads these endpoints. `--json` includes per-tool results and unexpected response shapes. A provider read failure is reported in the result. |
