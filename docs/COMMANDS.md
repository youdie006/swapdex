...
| `swapdex ui` | Interactive picker: every profile with its account and active marker, plus the session summary; type a number to switch (the same safe `use` path), Enter or `q` cancels. After the switch it lists that account's recent sessions with a `sessionwiki resume <id>` hint, so you land back in the work you switched for. Needs a terminal. |
