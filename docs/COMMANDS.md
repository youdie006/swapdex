...
| `swapdex ui` | Interactive picker: every profile with its account and active marker, plus the session summary; type a number to switch (the same safe `use` path), Enter or `q` cancels. After the switch it lists that account's recent sessions - pick one and swapdex hands off to `sessionwiki resume` (a one-shot `exec` of the official reopen flow; Enter skips). Switch, land back in your work, in one flow. Needs a terminal. |
