...
| `swapdex ui` | Full-screen picker (real terminal): arrow keys, Enter to switch, `a` add a new account (the sign-in flow), `r` restore, `d` delete, `q` quit. After a switch it lists that account's recent sessions - pick one and swapdex hands off to `sessionwiki resume`. When stdin is a pipe it falls back to a plain numbered prompt. |
