...
### Fixed
- **A Codex usage reading is credited to the account it was read from.** Codex writes its rate limits into the transcripts under a home directory, and swapdex was captioning each reading with whoever the switch timeline said was PAYING at the moment it was written. Those are different questions, and on a machine where one account serves while another's home holds the sessions they gave different answers: an account whose home holds no transcripts at all was shown carrying real numbers, while the home holding every one of them showed none. The reading belongs to the home it was found in. That is what swapdex can read today, not a claim that nothing better exists: a Codex gateway in the wild parses `x-codex-primary-used-percent` and friends off upstream responses and probes `/backend-api/codex/usage`, and neither has been tried here - this proxy has only ever looked for Anthropic's headers. What upstream will not do is name the account in the transcript; openai/codex#16323 asked for that and was declined.
